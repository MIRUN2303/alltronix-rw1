          ...
          <div className="relative max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-primary leading-tight mt-2">{category.name}</h1>
              </div>
            </div>
            <p className="text-lg text-secondary max-w-2xl mt-4">{category.description}</p>

            <div className="mt-16 space-y-4">
              {category.subcategories.map((sub, index) => (
                <ScrollReveal key={sub.name} delay={index * 0.05}>
                  <GlassCard className="p-6 lg:p-8 group" hover>
                    <h3 className="text-xl font-semibold text-primary group-hover:text-[#cc0000] transition-colors">{sub.name}</h3>
                    <p className="text-secondary mt-3 leading-relaxed">{sub.description}</p>
