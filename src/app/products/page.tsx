                ...
                <ScrollReveal key={cat.id} delay={index * 0.1}>
                  <Link href={`/products/${cat.slug}`}>
                    <GlassCard className="p-6 h-full group" hover>
                      ...
                      <h3 className="text-lg font-semibold text-[#1a1a1a] mb-2 group-hover:text-[#cc0000] transition-colors">{cat.name}</h3>
                      <p className="text-sm text-[#666] leading-relaxed mb-4">{cat.description}</p>
                      <p className="text-xs text-[#cc0000] font-medium">{cat.subcategories.length} product categories</p>
